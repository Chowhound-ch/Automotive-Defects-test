    ...
    <el-row>
      ...
      <el-col :span="18">
        <el-table v-loading="loading" :data="carList" @selection-change="handleSelectionChange" @sort-change="getListAll">
          ...
          <el-table-column label="id" width="60" align="center" prop="carId" sortable="custom" />
          <el-table-column label="厂商" width="55" align="center" prop="manufacturer" />
          <el-table-column label="型号" width="80" align="center" prop="ver" />
          <el-table-column label="价格（万元）" width="130" align="center" prop="price" sortable="custom" />
          <el-table-column label="上架时间" width="160" align="center" prop="createTime" sortable="custom">
            <template slot-scope="scope">
              {{formatDate(scope.row.createTime)}}
            </template>
          </el-table-column>
          <el-table-column label="审核状态" width="100" align="center" prop="isReviewed" sortable="custom">
            <template slot-scope="scope">
        ...
        <el-dialog title="审核信息" :visible.sync="showReview" width="900px" append-to-body>
          ...
          <el-row :gutter="10" class="review-row">
            <el-col :offset="12">
              <el-col :span="4">
                <el-button v-if="reviewForm.isReviewed < 4" type="primary" @click="handleReviewUpdate" >前往修改</el-button>
              </el-col>
              <el-col :span="4">
                <el-button v-if="reviewForm.isReviewed !== 4 && reviewForm.isReviewed !== 1" type="danger" @click="handleReview">提交审核</el-button>
              </el-col>
              <el-col :span="4">
                <el-button v-if="reviewForm.isReviewed < 3" type="info" @click="handleUnPublish">取消上架</el-button>
              </el-col>
            </el-col>
...
export default {
  ...
  methods: {
    ...
    handleReview() { // 重复提交
      request({
        ...
        data: {
          ...
          mileage: this.reviewForm.mileage,
          output: this.reviewForm.output,
        }
      }).then(res => {
        if (res.code === 200) {
          this.$message({
    ...
    handleReviewUpdate() { // 跳转修改
      localStorage.setItem('tempFormData', JSON.stringify(this.reviewForm));
      this.$router.push({ path: "/shop/publish", query: {operate: 'update' } });
    },
    handleUnPublish(){
      let params = {carId: this.reviewForm.carId, isReviewed: 3};
      this.$modal.confirm('是否确认下架id为"' + this.reviewForm.carId + '"的商品？').then(function() {
        return request({
          url: "/car/review",
          method: "POST",
          data: params
